...
namespace Zad3.Controllers
{
    ...
    public class EnrollmentsController : ControllerBase
    {
        ...
        private readonly ILinqService linq;
        public EnrollmentsController(IStudentDbService dbService, IConfiguration _configuration, ILinqService _linq)
        {
            service = dbService;
        ...
        {
            MyHelper helper = linq.addStudent(student);
            if (helper.Value != 0)
            {
        ...
        [HttpPost("promotions")]
        /*[Authorize(Roles = "employee")]*/
        public IActionResult promote(Enrollment study)
        {
            MyHelper helper = linq.promote(study);
            if (helper.Value != 0)
            {
                return BadRequest(helper.Message);
            }
        ...
        {
            return Ok(linq.listStudent());
        }

        [HttpPost("student")]
        public IActionResult updateStudent(Student student)
        {
            var helper = linq.updateStudent(student);
            if (helper.Value != 0)
            {
                return BadRequest(helper.Message);
            }
        ...
        {
            var helper = linq.deleteSteudent(id);
            if (helper.Value != 0)
            {
                return BadRequest(helper.Message);
            }
